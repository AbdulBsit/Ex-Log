...
def index():
    ...
    if request.method == "POST":
        if request.form.get('search'):
            ...
            r = requests.get(url="http://ide50-atiya-rabbi.cs50.io:8082/" + str(session['user_id']), json='true')
        ...
        url = "http://ide50-atiya-rabbi.cs50.io:8082/" + str(session['user_id'])
...
def url(user_id):
    ...
    if search != "false":
        ...
        interests = db.execute("SELECT interests FROM interest WHERE id=:Id", Id=user_id)
        if not interests:
...
def profile():
    ...
    dp = db.execute("select dp,username,count,name from users where id=:Id",
                    Id=session["user_id"])
    if request.method == "POST":
        dp = request.form.get("encodedImg")
        ff = request.form.get("firstfriend")
        fb = request.form.get("favouritebook")
        ins = []
        ins.append(request.form.get("business"))
        ins.append(request.form.get("entrepreneurship"))
        ins.append(request.form.get("politics"))
        ins.append(request.form.get("science"))
        ins.append(request.form.get("technology"))
        ins.append(request.form.get("entertainment"))
        ins.append(request.form.get("writing"))
        ins.append(request.form.get("health"))
        ins.append(request.form.get("self"))
        ins.append(request.form.get("fashion"))
    ...
    if request.method == "POST":
        ...
        # implementation of correct usage
            ...
            result = db.execute("INSERT INTO users (name, username, hash) VALUES(:name, :username, :hash)",
                                name=request.form.get("name"), username=request.form.get("username"), hash=hash)
